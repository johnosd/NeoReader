...
| Convergência (Fase 8) | Quase concluída (T070/T071/T073 fechados + 4 tasks ad-hoc T074-T077; só T072 — LOW, não relacionado — segue aberto). Cenário 2 do `quickstart.md` (self-hosted + Basic Auth) fechado ponta a ponta contra um Calibre real; SC-004 verificado empiricamente via inspeção direta do LevelDB no device. 4 mudanças novas: validar conexão antes de salvar catálogo (FR-027), ícone de status + atalho pra biblioteca em Settings (FR-028), capas autenticadas pra catálogo com credencial (FR-029), mensagem de erro específica no download (FR-030). `spec.md` sincronizado com FR-024 a FR-030. Suíte completa 747/747 (2 skipped pré-existentes), build limpo, reinstalado e confirmado funcionando ao vivo pelo usuário |
...
**PRÓXIMO**: `Phase 8: Convergence` quase fechada — só `T072` (LOW, reexercitar download a partir da busca no Gutenberg) segue aberto, não relacionado ao teste do Calibre desta sessão. Rodar `sdd-converge` de novo pra fechar formalmente (deve convergir limpo ou achar só o T072 residual).
