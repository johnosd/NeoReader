...
| User Story 1 (gesto/menu/pintura) | **Concluída, incluindo device (T024 completo em 2026-09-07, R-001 fechado)**. Gesto de seleção nativo + menu `#nr-selection-menu` + guarda `text-selection` no click (agora com leitura viva da seleção, R-010) + swatch resolvido por coordenada (R-011) + pintura via `Overlayer.highlight` com cor CSS (R-012) + wiring em `ReaderScreen.tsx` |
...
| Polish (Fase 7) | **Concluída** (T045-T049; T046-T048 verificados por revisão de código em 2026-09-07, T045/T049 no device na mesma data). Terminologia "highlight" consistente (zero "grifo"), FR-028a intacto (`VocabularyScreen` sem nada de highlights), os 3 comentários do Princípio II presentes, `README.md` ganhou a seção "Highlights" e a aba na tela de detalhes, R-001 fechado sem desvio |

## Riscos e Decisões

| ID | Risco/Decisão | Impacto | Mitigação/Encaminhamento |
| --- | --- | --- | --- |
| R-001 | CFI de intervalo pode não resolver entre sessões: o app injeta `<span class="nr-vocab">` no texto (`EpubViewer.tsx:706-722`) e insere o bloco de tradução com `para.after` — as duas coisas alteram a estrutura que o CFI endereça. O CFI de marcador escapa por ser colapsado no início do parágrafo; o de intervalo não tem essa folga | Alto para FR-013/SC-004: highlight poderia "sumir" ao reabrir o livro | **Fechado em 2026-09-07, sem desvio**: usuário criou um highlight cobrindo uma palavra salva no vocabulário do mesmo parágrafo, fechou o app por completo (swipe da lista de recentes) e reabriu — o highlight permaneceu no lugar certo (quickstart 1.5/5.5). O `<span class="nr-vocab">` não desloca o CFI de intervalo do highlight na prática. `text`/`paraCfi` continuam gravados (custo já pago) mas o fallback por texto **não foi implementado** — não há desvio que o justifique (Princípio III) |
...
| 2026-09-06 | 3 estilos de marcação — validação em device (T067b) | Aparelho reconectado (`RXCX103NMVZ`/SM-S911B); `npm run android:run` instalou sem erro. Usuário testou os 3 estilos nos dois menus (criar e gerenciar) e confirmou: "deu certo" | Fechamento do plano — feito na linha seguinte |
| 2026-09-07 | Fechamento do plano (T024, T045, T046-T049, T061c) | Usuário pediu conferência de progresso ("verifique o plano se concluimos"). Revisão achou 7 tasks abertas: 3 de desk-review sem device (T046 terminologia/FR-028a, T047 comentários do Princípio II, T048 README) fechadas na hora via leitura de código — tudo já conforme, só o README ganhou a seção "Highlights" que faltava. As 4 restantes exigiam device (T024/1.5, T045/Passo 5, T061c, T049) — usuário optou por rodar agora em vez de deixar pendente. Roteiro combinado de 3 blocos: (1) highlight sobre palavra do vocabulário sobrevive a fechar/reabrir o app — **fecha R-001**; (2) reflow de fonte/tamanho/tema, rotação e TTS com highlight visível — validado; (3) Compartilhar reconfirmado (sheet nativo com o texto certo, cancelar não trava). Os três blocos confirmados pelo usuário sem ressalva | Nenhuma — **89/93 → 93/93 tasks**, feature 010 completa incluindo Fase 7 de polimento |

**PRÓXIMO**: nenhum item aberto nesta feature.
