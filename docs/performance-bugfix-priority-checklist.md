...
| PBF-04 | Implementado localmente | Adicionados `tts.playback.start`, `tts.playback.pause`, `tts.playback.resume`, `tts.playback.stop` e `tts.playback.error` com `flowId`, provider, modo (`continuous`/`inline`) e motivo de parada quando aplicavel. Validado com `npm.cmd run test -- src/__tests__/hooks/useTTS.test.tsx`, `npm.cmd run test` e `npm.cmd run build` em 2026-06-14. Falta reexecutar AQA-09, AQA-16, AQA-17 e AQA-18 no Android para confirmar logs no dispositivo. |
| PBF-05 | Implementado localmente | `useReaderAppearance` registra `reader.appearance.fontSize.change`, `reader.appearance.theme.change`, `reader.appearance.lineHeight.change` e `reader.appearance.fontFamily.change` com `previousValue`/`nextValue`, sem texto do livro. Validado com `npm.cmd run test -- src/__tests__/hooks/useReaderAppearance.test.tsx`, `npm.cmd run test` e `npm.cmd run build` em 2026-06-14. Falta reexecutar AQA-10 no Android para confirmar logcat. |
| PBF-06 | Implementado localmente | Removido o import do SVG `neo-reader-header-logo` de 688 KB da Home; o logo agora usa `BookOpen`/CSS e o build nao emite mais esse asset. Validado com `npm.cmd run build` em 2026-06-14. O warning de chunk principal >500 KB ainda existe e deve virar etapa futura de code-splitting. |
